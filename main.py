...
def get_config():
    ...
    try:
        ...
        entry4.insert(0, data[2])
        entry20.insert(0, data[3])
        entry6.insert(0, data[4])
        entry8.insert(0, data[5])
        entry10.insert(0, data[6])
        entry12.insert(0, data[7])
        entry5.insert(0, data[8])
        entry7.insert(0, data[9])
        entry9.insert(0, data[10])
        entry11.insert(0, data[11])
        entry21.insert(0, data[12])
        entry50.insert(0, data[13])
        print(data[14])
    except:
        pass

def save_config_prepare():
    enabled = var1.get()
    ...
    custombind4t = entry11.get()
    custombind5t = entry21.get()
    string = entry2.get()
    string2 = entry3.get()
    string3 = entry4.get()
    save_config(string, string2, string3, custombind1, custombind2, custombind3, custombind4, custombind5, custombind1t, custombind2t, custombind3t, custombind4t, custombind5t, holdmouse, enabled)

def save_config(cps, start_htk, stop_htk, custombind1, custombind2, custombind3, custombind4, custombind5, custombind1t, custombind2t, custombind3t, custombind4t, custombind5t, holdmouse, enabled):
    file = open("config.txt", "w")
    file.write(str(cps) + "\n" + str(start_htk) + "\n" + str(stop_htk) + "\n" +  str(custombind1) + "\n" + str(custombind2) + "\n" + str(custombind3) + "\n" + str(custombind4) + "\n" + str(custombind5) + "\n" + str(custombind1t) + "\n" + str(custombind2t) + "\n" + str(custombind3t) + "\n" + str(custombind4t) + "\n" + str(custombind5t) + "\n" + str(holdmouse) + "\n" + str(enabled))
    #messagebox.showinfo("Clicker", "Config saved!")
    file.close()
...
l.place(x=0, y=270)
entry12 = tkinter.Entry(root, width=9)
entry12.place(x=93, y=270)
entry10 = tkinter.Entry(root, width=9)
...
entry8 = tkinter.Entry(root, width=9)
entry8.place(x=93, y=190)
entry50 = tkinter.Entry(root, width=9)
entry50.place(x=134, y=90)
entry20 = tkinter.Entry(root, width=9)
entry20.place(x=93, y=110)
entry6 = tkinter.Entry(root, width=9)
...
m = tkinter.Button(root, text = 'Apply', command=apply, bg="white", width = 11)
m.place(x=1, y=352)
m = tkinter.Button(root, text = 'Save', command=save_config_prepare, bg="white", width = 11)
m.place(x=100, y=352)
get_config()
root.mainloop()
